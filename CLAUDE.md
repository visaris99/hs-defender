...
- **Firebase Firestore** — 상담 신청 폼 데이터 저장 (클라이언트 직접 연결, API 라우트 없음)
...
lib/firebase.ts   → Firebase 초기화 (환경변수 기반, 미설정 시 graceful 처리)
...
- 상담 신청 폼: 3단계 위자드 → Firebase Firestore `applications` 컬렉션에 저장
...
`.env.local.example` 참고. Firebase 자격증명 6개, GTM ID, 사이트 인증 키, 카카오톡 채널 URL 등이 필요.

## 주의사항

- Tailwind v4의 `@theme` 문법 사용 — `tailwind.config` 파일이 없는 것이 정상
- 모든 페이지 컴포넌트가 `"use client"` — 서버 컴포넌트/SSR 패턴 미사용
- Firebase 직접 클라이언트 연결 — API 라우트 없음
